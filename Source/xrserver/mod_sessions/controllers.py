...
# view table list 
@mod_sessions.route('/getActList', methods=('GET', 'POST', 'PUT'))
def getList():
...
def getPrivateSessionList():
    if request.method == "POST":
        try:
            vvv=list(request.form.listvalues())
            #print(vvv)
            #print(len(request.form.getlist('name')))
        ...
        InvitePrivSelected = db.session.query(Session.session_id, Session.event_name, Session.access_type, Session.description, Session.start_date, 
                            Session.end_date, Session.environment_id, Session.category,Session.host_user_email, InviteeList).join(InviteeList, (InviteeList.invitee_email== email) & (Session.session_id == InviteeList.session_id) & (Session.access_type == '1')).filter(Session.start_date >= todays_datetime)
        if name!="" or name!='undefined':
            name = "%{}%".format(name)
            InvitePrivSelected=InvitePrivSelected.filter(Session.event_name.like(name))
        InvitePrivSelected=InvitePrivSelected.all()
        # b = Session.query.filter_by((Session.access_type =='public') | (Session.access_type =='private-1')).all()
        displayPrivateSelected = Session.query.filter_by(host_user_email = email, access_type ='1').filter(Session.start_date >= todays_datetime)
        if name!="" or name!='undefined':
            name = "%{}%".format(name)
            displayPrivateSelected=displayPrivateSelected.filter(Session.event_name.like(name))
        displayPrivateSelected=displayPrivateSelected.all()
        # displayPrivate = Session.query.filter_by(access_type ='1').all()
        displayPublicAll = Session.query.filter_by(access_type ='0').filter(Session.start_date >= todays_datetime)
        if name!="" or name!='undefined':
            name = "%{}%".format(name)
            displayPublicAll=displayPublicAll.filter(Session.event_name.like(name))
        displayPublicAll=displayPublicAll.all()
