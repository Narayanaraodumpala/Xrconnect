...
def getSessionList():
    if request.method == 'GET' or 'POST':
        ...
        user = db.session.query(
            ...
            Aviation.help
        ).filter_by(user_id=result).order_by(desc(Aviation.id)).all()
...
def getSessionListPagination():
    if request.method == 'GET':
        ...
        sessions=sessions.filter(Aviation.mode!='')
        if name is not None:
            search = "%{}%".format(name)
            sessions=sessions.filter(Aviation.user_name.like(search))

        sessions=sessions.group_by(Aviation.user_id, Aviation.session_id, Aviation.module, Aviation.mode)
        # .filter(Aviation.module!=None)
        total=0
        result=sessions.limit(per_page).offset(offset)
        result2=sessions.all()
        ...
        if result is not None:
            session_schema = AviationSchema()
            data = session_schema.dump(result, many=True)
...
def getSessionListDetails():
    if request.method == 'GET':
        ...
        if session_id is not None:
            sessions=sessions.filter_by(session_id=session_id)
        if module is not None:
            sessions=sessions.filter_by(module=module)
        if mode is not None:
            sessions=sessions.filter_by(mode=mode)

        sessions=sessions.order_by(desc(Aviation.id)).all()
...
def getSessionListbyProjectName():
    if request.method == 'GET' or 'POST':
        ...
        try:
             ...
             result = request.args.get('project_name')
        ...
        user = db.session.query(
            ...
            Aviation.project_name,
            Aviation.help
        ).filter_by(project_name=result).order_by(desc(Aviation.id)).all()
        print('session exist')
        print(user)
...
def getSessionListbyUser():
    if request.method == 'GET' or 'POST':
        ...
        user = db.session.query(
            ...
            Aviation.project_name,
            Aviation.help
        ).filter_by(user_name=result).order_by(desc(Aviation.id)).all()
...
def getSessionListbyDate():
    if request.method == 'POST':
        ...
        #                                  {"status": "fail", "message": ' from_date incorrect date string format. It should be YYYY-MM-DD HH:MM:SS', "data": ""}))

        except Exception as e:
            return make_response(
                jsonify(
                    {"status": "fail", "message": ' from_date and  to_date fields are  required', "data": ""}))
            
        # except ValueError :
        #      return make_response(
        #         jsonify(
        #             {"status": "fail", "message": '  fields are  required', "data": ""}))

        user = db.session.query(Aviation).filter(
            Aviation.date_created > from_date).filter(Aviation.date_created <= to_date).order_by(desc(Aviation.date_created))

        print('user data with daters=', user)
        ...
        #     print("This is the correct date string format.")
        # except ValueError:
        #     error=" to_date incorrect date string format. It should be YYYY-MM-DD HH:MM:SS "
        #     print("This is the incorrect date string format. It should be YYYY-MM-DD HH:MM:SS")
        #     responseObject = {"status": "fail", "data": "", "message": error}
        #     return make_response(jsonify(responseObject))
        ...
        if not user:
            ...
            error = "No existing data with this dates "
            responseObject = {"status": "fail",  "message": error}
            return make_response(jsonify(responseObject))
...
@mod_add_aviation_api.route('/getSessionListbyProjectNameandDate', methods=('GET', 'POST', 'PUT'))
def getSessionListbyProjectNameandDate(**kwargs):
    if request.method == 'POST':
        # user = User.query.order_by(desc(User.date_created)).all()
        try:
            ...
            return make_response(
                jsonify(
                    {"status": "fail", "message": 'to_date is required', "data": ""})
            )
        user = db.session.query(
            Aviation.id,
            Aviation.session_id,
            ...
            Aviation.project_name,
            Aviation.help
        ).filter_by(project_name=result).filter(
         Aviation.date_created > from_date).filter(Aviation.date_created <= to_date).order_by(desc(Aviation.date_created)).all()
        # filter_by(project_name=result).filter(
        #     Aviation.date_created > from_date).filter(Aviation.date_created <= to_date).all()
...
@mod_add_aviation_api.route('/getdate', methods=('GET', 'POST', 'PUT'))
def getcurrentdata():
    print('curerent date=',datetime.now())
    return make_response(jsonify(datetime.now()))
